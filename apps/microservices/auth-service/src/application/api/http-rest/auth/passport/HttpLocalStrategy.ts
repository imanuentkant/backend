...
export class HttpLocalStrategy extends PassportStrategy(Strategy) {
  constructor(private configService: ConfigService, private authService: HttpAuthService) {
    const LOGIN_USERNAME_FIELD = configService.get<string>('API_LOGIN_USERNAME_FIELD') || 'email';
    const LOGIN_PASSWORD_FIELD = configService.get<string>('API_LOGIN_PASSWORD_FIELD') || 'password';
