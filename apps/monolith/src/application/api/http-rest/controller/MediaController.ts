...
export class MediaController {
  ...
  private setFileStorageBasePath(medias: MediaUseCaseDto[]): void {
    medias.forEach((media: MediaUseCaseDto) => {
      return new URL(media.url, CoreAssert.notEmpty(this.configService.get<string>('FILE_STORAGE_BASE_PATH'), new Error('Minio: image base path not set'))).toString();
    });
  }
