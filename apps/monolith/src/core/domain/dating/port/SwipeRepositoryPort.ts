...
export interface SwipeRepositoryPort {
  ...
  findByTargetProfileId(targetProfileId: string): Promise<Swipe[]>;
  
  findLastSwipeByCustomer(customerId: string): Promise<Nullable<Swipe>>;
  
  delete(swipe: Swipe): Promise<void>;
}
